...
def recommend_exposure(aperture: float, iso: int, zone_choice: int,
    ...
    zones_map = {label: (zone_from_reading(ev, ev_final), ev) for label, ev in readings_ev.items()}

    return "\n".join(lines), ev_final, zones_map, t_std

def plot_zone_system(zones_map: dict, suggested_shutter: float):
    ...
    ax.text(5, 1.22, f"Suggested (Z5) {format_shutter(suggested_shutter)}", color=colors['Suggested'],
            ha='center', va='bottom', fontsize=10)
    for label, (zpos, ev) in zones_map.items():
        z = max(0, min(10, zpos))
        shutter_str = format_shutter(ev_to_shutter(ev))
        ax.plot(z, 1.08, 'o', color=colors.get(label, 'black'), markersize=8)
        ax.text(z, 1.20, f"{label} (Z{zpos:.1f}, {shutter_str})", color=colors.get(label, 'black'),
...
with colC:
    zone_choice = st.radio("🌑 Place darkest in:", [2, 3], index=0, horizontal=True)
...
with col1:
    brightest = st.select_slider("☀️ Brightest part", options=STANDARD_SHUTTERS, value=1/125, format_func=format_shutter)
    midtone   = st.select_slider("🌗 Mid-tone", options=STANDARD_SHUTTERS, value=1/60, format_func=format_shutter)
with col2:
    darkest = st.select_slider("🌑 Darkest part", options=STANDARD_SHUTTERS, value=1/30, format_func=format_shutter)
    subject = st.select_slider("🎯 Subject", options=STANDARD_SHUTTERS, value=1/125, format_func=format_shutter)
...
if st.button("📸 Calculate Exposure"):
    ...
    if ev_final is not None:
        fig = plot_zone_system(zones_map, t_std)
        st.pyplot(fig)
